...
- CD3+ Tissue-resident memory T cells: 1, 2, 4, 7, 10, 12, 16
- CD3+ NK-like cells (XCL1;XCL2+): 6, 24, 39
- CD3+ Regulatory T cells (LEF1+): 14
- CD3+ Proliferating T cells (TOP2A+): 35
- Myeloid cells: 3, 9, 15, 17, 18, 22, 31, 37, 40, 43
- Macrophages (CD5L+): 3, 22, 43
- CD14+/CD16+ Monocytes (S100A8+): 9, 18
- CD14-/CD16+ Non-classical monocytes: 15, 17
- cDC1 conventional DC type 1 (CST3+): 31
...
clusters <- gsub('\\b3\\b|\\b22\\b|\\b43\\b', "Macrophages",
                 clusters,
                 perl = TRUE)
clusters <- gsub('\\b14\\b', "CD3+ Regulatory T cells",
                 clusters,
                 perl = TRUE)
...
- CD14-/CD16+ Non-classical monocytes: 0, 14, 21, 24
- CD14-/CD16- Monocytes: 6, 9
- CD14+/CD16+ Monocytes: 8
- Monocytes 4 (PLAC8+/dendritic?): 14 
- Lymphocytes: 1, 2, 4, 5, 7, 10, 13, 15, 17
- CD3+/LEF1+/TCF7+ memory T cells: 1, 5, 10, 13, 17
- CD3+/CD8A+/NKG7+ NK-like cells (XCL1;XCL2+): 4, 7
- CD3+/MKI67+/TOP2A+ Proliferating T cells (dividing): 23
- CD3+/GATA3+/MAF+ TH2-like T cells 1 (GATA3+): 2
- CD3+/GATA3+/MAF+ TH2-like T cells 2: 15
- B-cells: 3, 11
- HSC: 16
- Antibody-secreting B cells: 18
- Megakaryocytes: 19
- Mast cell: 22
- Erythrocytes: 12
- Unknown: 20
...
```{r}
clusters <- as.character(Idents(sobj))
clusters <- gsub('\\b1\\b|\\b5\\b|\\b10\\b|\\b13\\b|\\b17\\b',
...
clusters <- gsub('\\b3\\b|\\b11\\b', "B cells",
                 clusters,
                 perl = TRUE)
clusters <- gsub('\\b2\\b', "CD3+/GATA3+/MAF+ TH2-like T cells 1",
                 clusters,
                 perl = TRUE)
clusters <- gsub('\\b4\\b|\\b7\\b', "CD3+/CD8A+/NKG7+ NK-like cells",
                 clusters,
                 perl = TRUE)
clusters <- gsub('\\b0\\b|\\b14\\b|\\b21\\b|\\b24\\b', 
                 "CD14-/CD16+ Non-classical monocytes",
                 clusters,
                 perl = TRUE)
clusters <- gsub('\\b23\\b', "CD3+/MKI67+/TOP2A+ Proliferating T cells",
                 clusters,
                 perl = TRUE)
clusters <- gsub('\\b15\\b', "CD3+/GATA3+/MAF+ TH2-like T cells 2",
                 clusters,
                 perl = TRUE)
clusters <- gsub('\\b6\\b|\\b9\\b', "CD14-/CD16- Monocytes",
                 clusters,
                 perl = TRUE)
clusters <- gsub('\\b8\\b', "CD14+/CD16+ Monocytes",
                 clusters,
                 perl = TRUE)
clusters <- gsub('\\b18\\b', "Antibody-secreting B cells",
                 clusters,
                 perl = TRUE)
...
clusters <- gsub('\\b2\\b', "CD3+ NK-like cells (2)", clusters, perl = TRUE)
clusters <- gsub('\\b6\\b', "CD3+ NK-like cells (6)", clusters, perl = TRUE)
