...
```{r}
cells_stim <- colnames(scSeurat)[(Idents(scSeurat) == 2 | Idents(scSeurat) == 6) & scSeurat$tissue_type == "6hrs_PMAIO"]
cells_unstim <- colnames(scSeurat)[(Idents(scSeurat) == 2 | Idents(scSeurat) == 6) & scSeurat$tissue_type == "6hrs_unstim"]
markers <- RunPresto(scSeurat,
                     ident.1 = cells_stim,
                     ident.2 = cells_unstim,
                     only.pos = FALSE,
...
```{r}
DimPlot(scSeurat, group.by = "general_cell_labels", label = TRUE) & NoLegend()
DimPlot(scSeurat, group.by = "integrated_snn_res.0.6", label = TRUE) & NoLegend()
FeaturePlot(scSeurat, features = "MARCO")
# Isolate macs
macs <- colnames(scSeurat)[scSeurat$general_cell_labels == "Myeloid" & (scSeurat$integrated_snn_res.0.6 == 5 | scSeurat$integrated_snn_res.0.6 == 15)]
monos <- colnames(scSeurat)[scSeurat$general_cell_labels == "Myeloid" & (scSeurat$integrated_snn_res.0.6 != 5 & scSeurat$integrated_snn_res.0.6 != 15)]
# Find markers
markers <- RunPresto(scSeurat,
                     ident.1 = monos,
                     ident.2 = macs,
...
Look for Th1 and Th2 signatures
Th1: CD4, STAT4, Tbet, Tnfb, ifng
Th2: CD4, GATA3, STAT6, IL 4/5/6/10/13
...
genes <- c("DHX8","JMJD1C","ERBIN","TCF12","HNRNPA2B1",
           "NFKB1","CD247","CSF1","IDO1","PDCD1",
           "CCL3L1;CCL3L3;CCL3;CCL18","CCL4L2;CCL4L1;CCL4","CCL5",
           "TNF","IFNG","TGFB1",
           "IL2RA","IL2","IL4","IL6","IL7","IL10","IL21")
...
FeaturePlot(scSeurat, feature = "ALB-1", split.by = "tissue_type")
```
